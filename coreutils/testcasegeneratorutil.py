...
parser.add_argument("-pl", "--payloads", type=str, action="store", dest="payloads", default="payloads/", help="provide payload location") 
parser.add_argument("-islsa", "--islsa", type=bool, action="store", dest="islsa", default=False, help="provide islsa True or False")


results = parser.parse_args() 
payloads= os.listdir(results.payloads)
islsa = results.islsa

rooth_file_path = os.path.abspath(os.path.join("payloads/", os.pardir))

#To generate the robot driver file to execute test cases.
def generate_robot_driver_file():

    testcases = os.listdir("testcases/")

    robot_sett_data = '''*** Settings ***
...
'''
    with open("drivers/unit_test_cases_driver.robot", "w+") as rbtobj: 
        rbtobj.write(robot_sett_data)

    with open("drivers/unit_test_cases_driver.robot", "a+") as rbtobj: 
        robot_test_data = '''
Validate {0} test-case
    ${{{1}_result}}=         Run Process  python testcases/{0}.py      shell=True
    Log                <a href="logs/{0}.log">{0}.log</a>                HTML
    Should Contain     ${{{1}_result.stdout}}      TESTCASE:PASSED for :{0}
'''
        for item in sorted(testcases):
            file_name = item.split(".")[0]
            if item.split(".")[0] == '' ".pyc" in item or "__init__" in file_name:
                continue
            else:
                rbtobj.write(robot_test_data.format(file_name, file_name.replace('-','_')))
...
@run_after(generate_robot_driver_file)
def generateTestCase(payloads):
    devices_lsa_input_list = [{ "deviceusername":"admin", 
                              ...
                              "devicename":"iso0",
                              "cfsipaddress":"127.0.0.1",
                              "cfsport":2026,
                              "cfsusername":"admin",
                              "cfspassword":"admin",
                              "rfsipaddress":"127.0.0.1",
                              "rfssshport":2026,
                              "rfsusername":"admin",
                              "rfspassword":"admin",
                              'islsa': islsa,
    ...
    devices_non_lsa_input_list = [{ "deviceusername":"admin", 
                              "devicepassword":"admin",
                              "requestmethod":"POST",
                              "createurl":"http://localhost:8080/api/running/dmz/services",
                              "deleteurl":"http://localhost:8080/api/running/dmz/services/interface-tunnel/Headend,Remote0/",
    ...
    with open("payloads/service_devices_mapping.py", "w+") as sdmobj: 
        for item in payloads:
            if item.split(".")[0] == '' or "service_devices_mapping" in item.split(".")[0] or "__init__" in item.split(".")[0] or ".pyc" in item:
                continue
            else:
                if islsa:
                    devices_per_service_mapping[item.split(".")[0]] = devices_lsa_input_list
                else:
                    devices_per_service_mapping[item.split(".")[0]] = devices_non_lsa_input_list
        # sdmobj.write("devices_per_service_mapping = "+str(pprint.pformat(devices_per_service_mapping, indent=1))) 
        sdmobj.write("devices_per_service_mapping = "+json.dumps(devices_per_service_mapping, indent=1))
           
    for item in sorted(payloads): 
        if item.split(".")[0] == '' or item.split(".")[0] == "service_devices_mapping" or "__init__" in item.split(".")[0] or ".pyc" in item:
            continue
        else:
            generateFile(item.split(".")[0]) 

def generateFile(payload): 
    fileName = "testcase-"+payload+".py" 
    f = open("testcases/"+fileName, "w+") 
    f.write(Template) 
    f.close()
